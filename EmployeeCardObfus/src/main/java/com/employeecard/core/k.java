...
import com.creditcard.validation.b;
...
public class k
{
  ...
  public j b(g paramg, int paramInt)
  {
    if (b(this.av).W() == 530)
    {
      ...
      if (this.av.y())
      {
        j = new j(300, "employee card account is successfully deposited");
        paramg.q(paramInt);
      ...
      {
        j = new j(320, "credit card declined to charge the amount");
      }
    }
  ...
  public j c(g paramg)
  {
    if (!this.aw.contains(paramg))
    {
      ...
      {
        this.aw.add(paramg);
        j = new j(400, "Employee card was registered for purchases");
        i.aq.ae()
      ...
      {
        j = new j(420, "credit card declined to charge the amount");
      }
      
    }
    else
    {
      j = new j(410, "Employee card was not registered for purchases because user already registered in the system");
    }
